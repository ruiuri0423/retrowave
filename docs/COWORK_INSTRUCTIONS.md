...
## 專案背景
- RetroWave 是一支「數位波型繪製工具」，主程式為單一 Python 檔 `retrowave.py`
...
1. 先用 `python -c "import ast; ast.parse(open('retrowave.py',encoding='utf-8').read())"`
   做語法檢查；若專案內有測試或煙霧測試腳本，一併執行，全部通過才繼續。
2. 更新版本號：把 `retrowave.py` 內的版本字串與相關說明同步到本次 `vX.Y`。
...
## 安全與邊界
- 推送目標固定為我的 repo `<owner/repo>`，分支 `<main>`；不要推到其他 repo 或分支。
...
## 文件與程式一致性檢查
- 每次收尾後，確認 `retrowave.py` 的版本、`RetroWave_Design.md` 的 Spec version/Changelog、
