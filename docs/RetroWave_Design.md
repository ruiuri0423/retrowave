...
- **v1.28** — **Group-header triangle drawn as a vector polygon.** The collapse/expand marker
  ...
  `create_polygon` (scaled by `export_scale`), so screen, PNG, and SVG render identically in
  any font environment. Also: project profile copy for listings (`docs/PROJECT_PROFILE.md`)
  and README hero/demo assets generated by RetroWave's own export pipeline (`assets/`).
